...
class Periodos extends Controller
{
	...
	public function getAll() {
        $periodos = Periodo::orderBy('anio')->orderBy('mes_inicio')->get();
        return $periodos;
    }
    ...
    public function getForAplicacion(Request $request) {
        $periodo = Periodo::where('anio', $request->input('anio'))->where('cuatrimestre', $request->input('cuatrimestre'))->select('id')->get();
        return $periodo;
    }
    ...
    public function validarPeriodicidad($anio){
        $cantPeriod = Periodo::where('anio', $anio)->count();
        return $cantPeriod;
    }
}
